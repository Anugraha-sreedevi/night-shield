...
export default function PlanJourneyView() {
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row h-full overflow-y-auto lg:overflow-hidden p-3.5 sm:p-5 md:p-6 gap-4 lg:gap-6 bg-[#F1F0FA] dark:bg-[#0D0D1A]">
      ...
      {/* Mobile View Toggle: Form vs Map */}
      <div className="lg:hidden flex items-center p-1 bg-white/90 dark:bg-[#1A1A32]/90 backdrop-blur-md rounded-full border border-slate-200/80 dark:border-white/10 shadow-xs shrink-0">
        ...
        >
          ...
          <span>Route & AI Risk ({matchedRoutes.length})</span>
        </button>
        <button
          type="button"
      ...
      <div className={`${mobileTab === 'plan' ? 'block' : 'hidden'} lg:block w-full lg:w-[460px] overflow-y-auto space-y-4 pr-1 pb-28 lg:pb-0`}>
      ...
      <div className={`${mobileTab === 'map' ? 'flex flex-col flex-1 h-[calc(100vh-170px)] sm:h-[calc(100vh-190px)]' : 'hidden'} lg:flex lg:flex-col lg:flex-1 lg:h-full relative`}>
        {/* Mobile floating top summary pill */}
      ...
      {/* "Why?" Explainable AI Modal via Portal */}
      {showWhyModal && riskData && createPortal(
        <div
          ...
          >
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm text-[#1B1B3A] dark:text-[#F4F3FD]">AI Risk Score Breakdown</h3>
              </div>
              <button
                onClick={() => setShowWhyModal(false)}
                className="w-7 h-7 rounded-full bg-slate-100 dark:bg-[#222144] hover:bg-slate-200 dark:hover:bg-[#2C2956] text-[#8A8AA8] dark:text-[#C5BFE0] flex items-center justify-center transition cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="text-xs text-[#8A8AA8] dark:text-[#9A9AB8] leading-relaxed">
              NightShield's RandomForest model evaluates 7 empirical features derived from 2,500 historical late-night commutes:
            </div>

            <div className="space-y-2.5">
              <div className="p-3 bg-[#F8F7FD] dark:bg-[#1E1B36] rounded-xl border border-slate-100 dark:border-white/10 text-xs">
                <div className="flex justify-between font-bold text-[#1B1B3A] dark:text-[#F4F3FD]">
                  <span>Late-Night Hour Multiplier</span>
                  <span className="text-[#8B5CF6] dark:text-[#A78BFA]">Weight: 2.8x</span>
                </div>
                <div className="text-[11px] text-[#8A8AA8] dark:text-[#9A9AB8] mt-0.5">
              ...
              <div className="p-3 bg-[#F8F7FD] dark:bg-[#1E1B36] rounded-xl border border-slate-100 dark:border-white/10 text-xs">
                <div className="flex justify-between font-bold text-[#1B1B3A] dark:text-[#F4F3FD]">
                  ...
                  <span className="text-[#EA580C] dark:text-[#FB923C]">
              ...
              <div className="p-3 bg-[#F8F7FD] dark:bg-[#1E1B36] rounded-xl border border-slate-100 dark:border-white/10 text-xs">
                <div className="flex justify-between font-bold text-[#1B1B3A] dark:text-[#F4F3FD]">
                  ...
                  <span className="text-[#16A34A] dark:text-[#34D399]">Lighting: {riskData.metrics?.lighting}</span>
                </div>
                <div className="text-[11px] text-[#8A8AA8] dark:text-[#9A9AB8] mt-0.5">
            ...
            <div className="p-3 rounded-xl bg-[#ECEAF8] dark:bg-[#2A2445] text-center text-[10px] font-bold text-[#8B5CF6] dark:text-[#A78BFA]">
              AI trained on simulated data &bull; Scikit-learn Random Forest (100 estimators)
            </div>

            <button
              onClick={() => setShowWhyModal(false)}
              className="w-full py-2.5 rounded-full gradient-violet-blue text-white text-xs font-bold shadow-btn hover:brightness-110 transition cursor-pointer"
            >
              Got it
            </button>
          </div>
